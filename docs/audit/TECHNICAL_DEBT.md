...
| --- | --- | --- |
| P0 | Native Windows rebuild, signing, and final packaged verification are blocked here by missing Developer Mode symlink support and no certificate. | Enable Developer Mode, run the fail-closed native build, configure Authenticode, and repeat clean-machine install/startup/upgrade/uninstall smoke. |
...
| P1 | Coverage is 69% overall and lifecycle modules remain weakly covered. | Add critical-module thresholds and worker/connector integration fixtures. |
...
| P1 | Performance has a synthetic 100k p95 baseline but no 24-hour/provider-scale evidence. | Add a scheduled scale run with CPU, RSS, database size, retry, and notification-duplication measurements. |
...
| P1 | OS-level retention/ACL verification remains. | Verify Credential Manager/ACL behavior and retention cleanup on a clean Windows account. |
